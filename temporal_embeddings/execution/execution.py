    ...
    def evaluator(self, split: str = "val") -> float:
        ...
        for batch in data_loader:
            with torch.cuda.amp.autocast(dtype=DTYPE):
                ...
                sent0_out = self.model.forward(input_ids=sent0_input_ids, attention_mask=sent0_attention_mask, date=batch.sent0_date)
                
                sent1_input_ids = batch.sent1.input_ids.to(DEVICE)
                sent1_attention_mask = batch.sent1.attention_mask.to(DEVICE)
                sent1_out = self.model.forward(input_ids=sent1_input_ids, attention_mask=sent1_attention_mask, date=batch.sent1_date)
